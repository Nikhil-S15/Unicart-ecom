const wishListModel = require("../../models/connection");
...
module.exports = {
  ...
  getWishListCount: (userId) => {
    try {
      return new Promise((resolve, reject) => {
        let count = 0;
        wishListModel.Wishlist.findOne({ user: userId }).then(
          (userWishlist) => {
            if (userWishlist) {
  ...
  addWishList: (userId, proId) => {
    try {
      return new Promise((resolve, reject) => {
        wishListModel.Wishlist.findOne({ user: new ObjectId(userId) }).then(
          (userWishList) => {
            if (userWishList) {
