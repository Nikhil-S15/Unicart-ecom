...
module.exports = {
  ...
  changeUserStatus: (userId, status) => {
    return new Promise((resolve, reject) => {
      try {
        dbAdmin.user
          .updateOne({ _id: userId }, { $set: { status: status } })
          .then((response) => {
            if (response) {
              resolve(response);
            } else {
  ...
  postAddCategory: (data) => {
    ...
    try {
      return new Promise((resolve, reject) => {
        ...
        categoryModel.Category.findOne({ category: category }).then(
          async (categoryDoc) => {
            if (!categoryDoc) {
                ...
                sub_category: [
                  {
                    ...
                    offer: { validFrom: 0, validTo: 0, discountPercentage: 0 },
                  },
                ],
              });
              ...
              let subcategoryDoc = categoryDoc.sub_category.find(
                (sub_category) => sub_category.name === data.sub_category
              );
              if (!subcategoryDoc) {
                categoryDoc.sub_category.push({
                  name: data.sub_category,
                  offer: { validFrom: 0, validTo: 0, discountPercentage: 0 },
                });
  ...
  deleteImage: (proId) => {
    try {
      return new Promise((resolve, reject) => {
        dbAdmin.Product.findByIdAndDelete({ _id: proId }).then(
          (response) => {}
        );
      });
    } catch (error) {
  ...
  getAllOrder: () => {
    try {
      return new Promise(async (resolve, reject) => {
        let Order = await dbAdmin.Order.aggregate([
          { $unwind: "$orders" },
          { $sort: { _id: -1 } },
        ]).then((response) => {
          resolve(response);
